 ...
 */
  ...
  static expandirCollapsesDestacados() {
    // Expande todos os .panel-info
    $('.panel-info').each((index, element) => {
      $(element).find('.panel-collapse').first().addClass('in');
    });
  
    // Expande todos os painéis que tem as duas classes .panel-approval e .panel-default ao mesmo tempo
    $('.panel-approval.panel-default').each((index, element) => {
      $(element).find('.panel-collapse').first().addClass('in');
    });
  }
